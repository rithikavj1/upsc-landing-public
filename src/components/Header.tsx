    ...
    <header className="sticky top-0 z-40 w-full border-b border-white/5 bg-brand-dark-bg/60 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center space-x-8">
          ...
          <nav className="hidden md:flex space-x-6 text-xs font-semibold text-zinc-400">
            <button
            ...
            <button
              onClick={() => document.getElementById("ai-interviewer")?.scrollIntoView({ behavior: "smooth" })}
              className="hover:text-brand-purple transition-colors flex items-center gap-1 text-brand-purple"
            >
              <span className="relative flex h-1.5 w-1.5">
                <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-brand-purple opacity-75"></span>
                <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-brand-purple"></span>
              </span>
              AI Mock Panel
            </button>
            <button
